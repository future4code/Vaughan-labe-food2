...
export default function RestaurantCard({ restaurant, display, height }) {
    ...
    return (
        <>
            {restaurant &&
                <RestaurantCardStyled key={restaurant.id} onClick={() => goToRestaurant(navigate, restaurant.id)} sx={{ height: `${height}` }}>
                    <img src={restaurant.logoUrl} alt="Imagem da logo" />
                    <Typography variant="h2" color="primary">{restaurant.name}</Typography>
                    <div>
                        <Typography variant='p' color="secondary" sx={{ display: `${display}`, m: "0 16px 0 16px" }}>{restaurant.category}</Typography>
                        ...
                        <Typography variant='p' color="secondary" sx={{ display: `${display}`, m: "8px 16px 0 16px" }}>{restaurant.address}</Typography>
                    </div>
